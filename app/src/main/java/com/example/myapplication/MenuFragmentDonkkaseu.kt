...
class MenuFragmentDonkkaseu : Fragment() {
    // 리스트 등록
    val MenuClass = MenuList()
    val donkkaseuMenu = MenuClass.menuData.donkkaseuList


    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceStage: Bundle?): View? {
        val view = inflater.inflate(R.layout.menu_fragment_donkkaseu, container, false)
    ...
    fun setTableRowLayout(view: View) {
        val tableRowLayoutOne = view.findViewById<TableRow>(R.id.donkkaseumenu_tablerow_one)
        val tableRowLayoutTwo = view.findViewById<TableRow>(R.id.donkkaseumenu_tablerow_two)
        ...
        for (index in 0 until 4) {
            ...
            val valueImage = requireContext().resources.getIdentifier(donkkaseuMenu[index].image, "drawable", requireContext().packageName)

            Glide.with(this).load(valueImage).into(customViewImage)
            customView.findViewById<TextView>(R.id.menu_text).text = donkkaseuMenu[index].name + "\n" + donkkaseuMenu[index].price.toString()
            customView.findViewById<Button>(R.id.menu_btn).setOnClickListener {
                // 장바구니 데이터클래스 설정
                val basketObj = BasketObj(valueImage, donkkaseuMenu[index].name, donkkaseuMenu[index].price)
